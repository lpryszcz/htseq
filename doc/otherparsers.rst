...
The `BigWig format`_ is a binary, compressed version of both Wiggle and bedGraph.
``HTSeq`` supports it via `pyBigWig`_ (a great library, btw, thank you!), mainly for use
with :class:`GenomicArray` instances, i.e. sparse data on genomic intervals.

.. _`BigWig format`: http://genome.ucsc.edu/goldenPath/help/bigWig.html
.. _`pyBigWig`: https://github.com/deeptools/pyBigWig

.. class:: BigWig_Reader(filename)
...
Methods
   ...
   .. method:: intervals(self, chrom, strand='.', raw=False)
      ...
      Args:
          ...
          ``strand ('.', '+', or '-')``: Strandedness of the yielded :class:`GenomicInterval`. If raw=True, this argument is ignored.
